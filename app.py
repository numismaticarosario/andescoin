...
3) 'referencia': Revisar la leyenda 'REFERENCES'.
   - Si contiene 'KM #<n>', devolver exactamente 'KM #<n>'.
   - Si no hay KM, buscar 'Y #<n>' y devolver 'Y #<n>'.
   - Si no hay Y, usar 'N #<n>' desde 'NUMBER' (si existe) y devolver 'N #<n>'.
   IMPORTANTE: Siempre devolver en formato: sigla (KM/Y/N) + espacio + '#' + número, sin nada más.
   Si no hay ningún valor, dejar cadena vacía.

4) 'metal': Tomar desde 'COMPOSITION' y traducirlo al español.

5) 'diametro': Tomar desde 'DIAMETER'. Devolver como '<numero> mm' (agregar 'mm' si faltara).
